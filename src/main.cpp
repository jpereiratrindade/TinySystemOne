...
void print_banner() {
    ...
    std::cout << "  (Strict Partitioning by Unique Canonical States — Zero Data Leakage)\n";
    std::cout << "======================================================================\033[0m\n\n";
}
...
int main() {
    ...
    std::cout << std::format("  • Universo Canônico Ω (576 estados únicos): Treino = {}, Val = {}, Teste = {}, OOD = {}\n\n",
    ...
    std::cout << "\033[1m[2] Treinamento sobre Estados Únicos (AdamW lr=0.008)\033[0m\n";
    std::cout << "--------------------------------------------------------------------------------------\n";
    ...
    // 3. Test Set Evaluation on Genuinely Unseen Configurations
    std::cout << "\033[1m[3] Avaliação de Generalização em Configurações Genuinamente Inéditas\033[0m\n";
    ...
    std::cout << std::format("  • Acurácia de Generalização: \033[1;32m{:.2f}%\033[0m\n", test_report.accuracy * 100.0f);
    ...
    // 4. Categorized OOD Evaluation
    std::cout << "\n\033[1m[4] Avaliação Estruturada da Taxonomia de OOD\033[0m\n";
    for (std::size_t i = 0; i < data_split.ood.size(); ++i) {
        ...
        std::cout << std::format("  ↳ Predição Top-1: \033[1;35m{}\033[0m (Confiança: {:.1f}%, Entropia Normalizada H: {:.2f})\n\n",
                                 tso::to_string(static_cast<tso::Choice>(pred_idx)), conf * 100.0f, norm_entropy);
    }

    std::cout << "\033[1;32m[✓] Experimento EXP-001 concluído com sucesso e rigor metodológico!\033[0m\n";
    return 0;
}
